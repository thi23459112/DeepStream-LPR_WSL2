...
RTSP_PORT   = 8554                                  # RTSP 伺服器監聽埠號（多路共用，以 mount_path 區分）
...
# 抖動過濾與容忍參數
# ------------------------------------------------------------
MOVEMENT_THRESHOLD = 70                             # Y 軸位移門檻（像素），位移 > 此值為 IN，< -此值為 OUT，否則為抖動
...
# RTSP 推流 ⭐
# ------------------------------------------------------------
# 連線範例：rtsp://<邊緣設備 IP>:<port>/<mount_path>
rtsp_push:
  enable: true                        # 可選值：true / false
  port: {RTSP_PORT}                          # RTSP 伺服器監聽埠號
  mount_path: "{source_id}"   # URL 路徑名稱（rtsp://IP:port/{{mount_path}}）
  bitrate: 4000000                    # 位元率 (bps)，1080p 建議 4M~8M
  encoder: "h264"                     # 可選值：h264（高相容性，推薦）/ h265（檔案較小）
