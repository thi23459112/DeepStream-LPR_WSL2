"""
類別顏色與標籤對應表（車牌辨識 LPR 版）

主要功能：
1. 提供車種類別 ID → OSD bbox 顏色對應 (RGBA)
2. 載入 labels_car.txt 並建立車種類別 ID → 車種名稱對應表
3. 載入 labels_num.txt 並建立字元類別 ID → 字元名稱對應表
   (供 probes.py 的 assemble_plate_probe 組裝車牌字串用)
4. 啟動時自動初始化 CLASS_MAP 與 NUM_MAP
"""
...
LABEL_CAR_FILE  = f"{BASE_DIR}/labels_car.txt"
LABEL_NUM_FILE  = f"{BASE_DIR}/labels_num.txt"
...
def load_labels(label_path):
    """
    從 labels.txt 載入類別 ID → 名稱對應表

    檔案格式：每行一個類別名稱，行號即為類別 ID (從 0 起算)

    用途：
        labels_car.txt → CLASS_MAP（車種對應，給 OSD 顯示與 DB 寫入）
        labels_num.txt → NUM_MAP（字元對應，給 assemble_plate_probe 組裝車牌字串）
    ...
    返回：
        dict: {class_id: class_name}
              檔案不存在時返回空 dict (上層會 fallback 顯示原始 ID)
    """
    class_map = {}
...
# 模組載入時自動建立 CLASS_MAP 與 NUM_MAP，給 probes.py 直接 import 使用
# CLASS_MAP : 車種對應（7 類）  → probes.py 在 OSD 顯示車種、_finalize_one 寫進 DB Class 欄位
# NUM_MAP   : 字元對應（36 類） → probes.py 在 assemble_plate_probe 把字元類別 ID 組成車牌字串
CLASS_MAP = load_labels(LABEL_CAR_FILE)
NUM_MAP   = load_labels(LABEL_NUM_FILE)